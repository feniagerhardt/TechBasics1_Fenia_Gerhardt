...
#st.write(df.columns.tolist()) # (activate if df does not work)
...
if df is not None and not df.empty:
    ...
    st.data_editor(filtering, key="v2")
    st.session_state["scene"] = sortscene
    add_line_button(key_suffix="normal_list")
    #st.write("DEBUG - full df:", st.session_state["df"])
...
def current_status():
    current_scene = st.session_state.get("scene", "")
    current_shot = st.session_state.get("numbershot", "")
    return current_scene, current_shot
...
if df is not None and filtering is not None:
    ...
    current_scene, current_shot = current_status()
    locator = (df["Scene"] == current_scene) & (df["Shot Number"] == current_shot)
    ...
    with col1:
        ...
        if click2:
            current_scene, current_shot = current_status()
            if current_scene is not None and current_shot is not None:
    ...
    with col2:
        ...
        if click:
            current_scene, current_shot = current_status()
            if current_scene is not None and current_shot is not None:
                df.loc[locator, "Shot Size"] = click #AI
        ...
        if click == "MCU (Medium Close Up)":
            st.badge("Simple but effective", color="red")
